<template>
    <i-table :opts="state.opts" ref="i-table">
        ...
        <template #i-name="{ scope = {} }">
            <span v-on:dblclick="method.edit(scope)" class="d-flex align-items-center">
                <el-avatar v-if="!utils.is.empty(scope.image)" :src="method.imageSize(scope?.image)" shape="square" size="small" class="me-1"></el-avatar>
    ...
    <el-dialog v-model="state.item.dialog" class="custom" draggable :close-on-click-modal="false">
        <template #header>
            <strong class="flex-center">{{ utils.is.empty(state.struct.id) ? '添 加' : '编 辑' }} 徽 章</strong>
        </template>
        <template #default>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        <label class="form-label required">
                            <el-tooltip content="（必须）徽章的名字" placement="top">
                                <span>
                        ...
                        <el-input v-model="state.struct.name" placeholder="徽章名称"></el-input>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        <label class="form-label">
                            <el-tooltip content="可以给徽章设置一个头像" placement="top">
                                <span>
                                    <i-svg color="rgb(var(--icon-color))" name="hint" size="14px"></i-svg>
                                    <span class="ms-1">徽章图标：</span>
                                </span>
                            </el-tooltip>
                        </label>
                        <el-input v-model="state.struct.image" class="custom" placeholder="填写图片地址或点击上传图片">
                            <template #prefix>
                                <el-avatar v-if="!utils.is.empty(state.struct.image)" :src="method.imageSize(state.struct.image, '50x50')" shape="square" size="small"></el-avatar>
                            </template>
                            <template #suffix>
                                <el-button v-on:click="method.upload('image')" :loading="state.item.upload">
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="form-group mb-3">
                        <label class="form-label">
                            <el-tooltip content="这个徽章的描述" placement="top">
                                <span>
                                    <i-svg color="rgb(var(--icon-color))" name="hint" size="14px"></i-svg>
                        ...
                        <el-input v-model="state.struct.description" :autosize="{ minRows: 3, maxRows: 10 }" type="textarea" placeholder="徽章描述信息"></el-input>
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="form-group mb-3">
...
import { useUsersStore } from '{src}/store/users.js'
...
const method = {
    ...
    save: async (params = state.struct || {}) => {
        ...
        if (utils.is.empty(params?.name)) return notyf.warn('徽章怎么能没有名字呢！')
